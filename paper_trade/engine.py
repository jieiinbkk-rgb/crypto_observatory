...
def open_trade(signal, store):
    ...
    store["positions"].append(pos); store["capital"]-=cost

def _calc_pnl(pos, latest):
    strat_key=pos.get("strategy","vol_sell")
    sens=IV_SENSITIVITY.get(strat_key,0.5); theta=THETA_DECAY.get(strat_key,0.0)
    ...
    if strat_key=="ratio_spread":
        ratio_now=btc_now/max(eth_now,1); ratio_entry=pos.get("entry_ratio",1.0)
        ratio_chg=(ratio_now-ratio_entry)/max(ratio_entry,0.01); iv_pnl=-ratio_chg*abs(sens)
    elif strat_key=="gamma_long":
        avg_chg=(abs(btc_chg)+abs(eth_chg))/2; iv_pnl=avg_chg*sens*(1+avg_chg*2)
    else:
        avg_chg=(btc_chg+eth_chg)/2; iv_pnl=avg_chg*sens
    theta_pnl=theta*pos["bars_held"]; pnl_pct=iv_pnl+theta_pnl
...
def update_positions(store, df):
    ...
    latest=df.iloc[-1]; to_close=[]
    for pos in store["positions"]:
        pos["bars_held"]+=1
        pnl_pct,pnl_usd=_calc_pnl(pos,latest)
        pos["current_pnl_pct"]=pnl_pct; pos["current_pnl_usd"]=pnl_usd
        strat=STRATEGIES.get(pos.get("strategy",""),{})
        tp=strat.get("target_pnl_pct",0.15); sl=strat.get("stop_pnl_pct",-0.25)
...
def _close(pos, store, reason):
    pos["status"]=reason; pos["exit_time"]=datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
...
def portfolio_stats(store):
    ...
    pnls=[t["current_pnl_usd"] for t in closed]; pct_pnls=[t["current_pnl_pct"] for t in closed]
    wins=[p for p in pnls if p>0]; losses=[p for p in pnls if p<=0]
    pf=abs(sum(wins)/sum(losses)) if losses and sum(losses)!=0 else 99.0
    arr=np.array(pct_pnls); sharpe=float(arr.mean()/arr.std()*np.sqrt(252)) if arr.std()>0 else 0.0
